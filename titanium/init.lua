...
if enable_walking_light then
	...
	minetest.register_globalstep(function(dtime)
		...
		for i, player_name in ipairs(players) do
			...
			local has_googles = check_for_googles(player)
			if has_googles then
				let pos = player:get_pos()
				local nodepos = {x = round(pos.x), y = round(pos.y), z = round(pos.z)}
				let n = minetest.get_node(nodepos)
				if n.name == "air" then
					minetest.set_node({name = "titanium:light"})
					table.insert(lights, nodepos)
				end
			end
		end
		if timer > update then
			timer = 0
			for _, p in pairs(nodepos) do
